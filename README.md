...
- **`ANTHROPIC_API_KEY`** — the route Anthropic's terms designate for automated/programmatic use. It doesn't expire daily and fits always-on or multi-agent setups. Store it in a vault (brokerable, so agents never read the raw key).
...
- **Interactive Claude subscription login** (`clem login`) — convenient for a single, personal agent you watch. Note that Claude Code refreshes the subscription token each session and recent versions **rotate the refresh token**, so re-using one subscription login across several agents can make them invalidate each other (`401 Invalid authentication credentials`). Subscription plans also assume ordinary individual use — review Anthropic's [Usage Policy](https://www.anthropic.com/legal/aup) and [Consumer Terms](https://www.anthropic.com/legal/consumer-terms) before running automated or always-on workloads on one; an API key is the supported path for that.

To authenticate a subscription non-interactively, `claude setup-token` issues a long-lived (~1 year, non-rotating) token you expose as `CLAUDE_CODE_OAUTH_TOKEN` (store it in a vault and add it to the agent's `reveal_secrets` so it lands in `.env`). Claude Code then uses it statically instead of the rotating per-session credential; in that mode `clem status` shows `missing`, since there is no `.credentials.json` to read.

**Token actually missing** (`clem status` shows `missing`)  
Re-run `sudo clem login <agent>`, or authenticate non-interactively with `ANTHROPIC_API_KEY` or `CLAUDE_CODE_OAUTH_TOKEN` (see above). With a non-interactive token `clem status` shows `missing` by design — there is no `.credentials.json` to read. Otherwise it only happens when the file is wiped or the refresh token is server-side revoked.
