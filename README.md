...
You probably ran a clem older than v0.8.4. The Claude Max *access* token lasts only about 8 hours in practice; Claude Code refreshes it automatically from the refresh token stored alongside it. Pre-0.8.4 `clem status` displayed the access-token expiry and pre-0.8.4 `NeedsLogin` gated on a 7-day window - so it always reported "expired" and trained operators to log in daily for nothing. Upgrade to v0.8.4+; status now shows `auto-refresh` whenever a refresh token is present, and only reports `missing` when manual `clem login` is actually required.

**Authenticating agents — API key, local model, or subscription login**  
clem runs the official runtime with whatever credential you provide, and each agent authenticates independently. Pick per your use:
- **`ANTHROPIC_API_KEY`** — doesn't expire daily and fits automated, always-on, or multi-agent use. Store it in a vault (brokerable, so agents never read the raw key).
- **Local or non-Anthropic models** — set `runtime: opencode` (or point claude-code at Ollama) to run a local or third-party model with no Anthropic auth at all.
- **Interactive Claude subscription login** (`clem login`) — convenient for a single, personal agent you watch. Several agents on one Claude account is unreliable: their sessions interfere when the login refreshes, and `401 Invalid authentication credentials` failures are common — for more than one agent, prefer an API key or a single non-interactive token (below). Subscription plans also assume ordinary individual use, and Anthropic's [Usage Policy](https://www.anthropic.com/legal/aup) and [Consumer Terms](https://www.anthropic.com/legal/consumer-terms) allow automated or always-on access only through the API — review them before running such workloads on a subscription.

To authenticate a subscription non-interactively, `claude setup-token` issues a long-lived (~1 year) token you expose as `CLAUDE_CODE_OAUTH_TOKEN` (store it in a vault and add it to the agent's `reveal_secrets` so it lands in `.env`). Claude Code uses it statically, so it isn't subject to the per-session refresh; `clem status` then shows `missing` because clem only reads the OAuth credentials file, which this path never creates.

**Token actually missing** (`clem status` shows `missing`)  
Re-run `sudo clem login <agent>`, or authenticate non-interactively with `ANTHROPIC_API_KEY` or `CLAUDE_CODE_OAUTH_TOKEN` (see above). `clem status` reads only the OAuth credentials file, so it shows `missing` for those non-interactive paths by design. Otherwise a genuinely missing credential means the file was wiped or the refresh token was server-side revoked.
